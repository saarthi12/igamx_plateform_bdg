...
<?php
	...
	if(isset($_POST['serial'])){
		...
		if($chkserialrow==1){
			$chkserial_ad = mysqli_query($conn,"select * from `tb_agent` where `mobile`='".$_POST['serial']."' AND `status`='1'");
			$chkserialrow_ad = mysqli_num_rows($chkserial_ad);
			if($chkserialrow_ad==0){
				...
				$tathya = mysqli_query($conn,"INSERT INTO `tb_agent` (`userid`,`mobile`,`createdate`,`status`) VALUES ('".$last_id."','".$serial."','".$createdate."','".$status."')");
				
				if($tathya){
					echo '<script type="text/JavaScript"> alert("Agent Added"); </script>';
				}else {echo '<script type="text/JavaScript"> alert("Agent Add Failed"); </script>';}	
			}
			else{
				echo '<script type="text/JavaScript"> alert("Agent exist"); </script>';
			}
		}
		else{
			echo '<script type="text/JavaScript"> alert("Mobile Doesn\'t exist"); </script>';
		}
	}
	if(isset($_POST['redserial'])){
		$a_id = $_POST['redserial'];
		$ch_s1 = "UPDATE tb_agent SET status='2' WHERE userid='".$a_id."'";
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
        <div class="content-wrapper">
		  ...
		  <div class="row">
			<div class="col-sm-12 mb-4 mb-xl-0">
              <h4 class="font-weight-bold text-dark" style="margin-top: 10px;">List of Agent Users</h4>
            </div>				
		  </div>
		  <div class="row">					
            <?php
			...
			?>
					<input name="redserial" type="radio" value="<?php echo $row['userid']; ?>" style="margin-top: 10px; margin-left:12px;" />
					<label for="<?php echo $row['userid']; ?>"><?php echo $row['userid']; ?> --- <?php echo $row['mobile']; ?></label>
					<br>
			<?php						
				}
			?>
