<script>
	if (!localStorage.getItem('userInfo')) {
		window.location.href = 'https://BDGGAMEBysXm/#/login';
	}
	else{
		...
		.then(data => {
			if(data.res != '1'){
				window.location.href = 'https://bdggame.sxmpro.in.net/';
			}
			else{
				let date = new Date();
      ...
      <div class="main-panel">
		...
		<footer class="footer">
			<div class="d-sm-flex justify-content-center justify-content-sm-between">
				<span class="text-muted d-block text-center text-sm-left d-sm-inline-block">Copyright © BDGGAMEBysXm</span>
			</div>
		</footer>
      </div>     
    </div>
